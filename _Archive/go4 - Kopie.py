...
while run:
    ...
    #if board[3][3] != 0:
    #    print(freedoms(board, 3, 3))

##    if last_placed:
##        print(freedoms(board, last_placed[1], last_placed[0]))
    ...
    # draw cursor
    pygame.draw.circle(fenster, (80, 50, 40), (offset+gx*grid_size, offset+gy*grid_size), rund(2/5*grid_size), 4)
    ...
    for y, row in enumerate(board):
        for x, spot in enumerate(row):
            if spot != 0:
                pygame.draw.circle(fenster, colors[spot], (offset + x*grid_size, offset + y*grid_size), rund(2/5*grid_size))
        
    # Weitere Anzeigen
    pygame.draw.circle(fenster, colors[players[turn]], (offset + (grid+1)*grid_size, offset+grid_size), rund(2/5*grid_size))
